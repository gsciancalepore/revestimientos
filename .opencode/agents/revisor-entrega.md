...
> Mirror para OpenCode de `.claude/agents/revisor-entrega.md`. La fuente versionada
> es `.claude/agents/`; si divergen, manda `.claude/`. A diferencia de los otros
> dos mirrors, este permite `edit` porque su chequeo distintivo (punto 1) exige
> mutar temporalmente la implementación y restaurar el árbol, siempre con
> `git status` limpio al cerrar.
