...
## Todo `Log::` nuevo es un evento del contrato de logs
No se escribe `Log::info('texto')` en `app/`: se emite `EventLog::record()` con un evento declarado en `docs/observabilidad/log-schema.v1.json`, sin datos personales. Ver `.ai/rules/observabilidad.md`.
