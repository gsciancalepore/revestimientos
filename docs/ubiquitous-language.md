...
| **Vista depósito** | Pantalla del panel para quien arma los envíos: dos solapas —*por preparar* (pedidos pagados) y *despachados*—, ordenadas por antigüedad y **sin importes**, con lo necesario para armar el paquete. Desde ahí se marca *despachado* y *entregado*. | `/admin/despacho` (Spec 08, regla 163) |
...
| **Webhook** | Aviso HTTP que MercadoPago manda al sistema cuando algo pasa con un pago, sin que nadie lo pida. Es lo que permite que un pedido pagado avance **solo**, sin que el admin mire el panel de MercadoPago. Su contenido **no se cree**: de él se toma solo el identificador y el estado real se consulta contra la API. | `POST /webhook/mercadopago` (Spec 08, reglas 153-158) |
| **Firma del webhook** | Sello con el que MercadoPago acredita que el aviso lo mandó él y no un tercero. Se valida con un secreto propio, **distinto del access token**. Sin firma válida el aviso se rechaza sin mirar su contenido. | Aviso sin firma o con firma inválida → 401 |
| **Incidente de pago** | Pedido con plata cobrada que **no puede avanzar**: el monto no coincide con el total, o el cobro llegó sobre un pedido cancelado. Queda trabado a propósito y se resuelve fuera del sistema, caso por caso. No se apaga solo. | Spec 08, reglas 151 y 157 |
