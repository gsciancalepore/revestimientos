...
> **⏸️ Staging parkeado (decisión del dueño, 2026-09-11):** hasta alcanzar el MVP funcional el foco
> es enteramente local. Nada de la Spec 08 lo necesita: el túnel para verificar MercadoPago también
> corre en la máquina local. Lo de abajo queda pendiente para el próximo deploy manual.
>
> **⚠️ Verificación pendiente (2026-09-10, auditoría documental):** el repositorio tiene **15
> migraciones**; el estado registrado abajo es un snapshot del `2026-09-01` con `11`. Como las
> migraciones son un step manual (§11) y no corren en el `CMD`, **no consta que
> `orders`/`order_lines`/`add_mp_fields_to_orders` estén aplicadas en Neon**. Si no lo están, el
> checkout de staging falla. Verificar con `php artisan migrate:status` contra Neon y actualizar
> este documento con el resultado.
>
