...
- **Fuentes**: Spec 07.1 (reglas 101–107, estructura `orders`/`order_lines`, `OrderStatus`, `PaymentGateway name()` solo), Spec 00 (19–23 pedido, 24–26 pagos, 27 anónimo), Spec 05 (81–92 carrito, 88 subtotal), Spec 06 (93–100 envío, `ShippingCalculator` + `ShippingQuote`), ADR-003 (bcmath, centavos), ADR-005 (stock al confirmar pago, `lockForUpdate`), ADR-004 (audit), ADR-006 (puertos)
...
108. **`PlaceOrderAction` entrada**: `execute(customer_name: string, customer_email: string, customer_phone: string, shipping_cp: string, shipping_address: ?string, payment_method: string): Order`. Valida `customer_name/phone` requerido string, `customer_email` formato email, `shipping_cp` `^[0-9]{4}$` string con ceros, `payment_method in ['transferencia','mercadopago']`, `cart no vacío` y **ninguna línea no comprable** (`Cart::hasUnpurchasable()` → `DomainException`). Ingreso `m²` ya fue derivado a `cajas` en `Cart` (regla 84), aquí solo `cantidad` entera.

109. **Transacción y lock**: todo dentro de `DB::transaction` con `Product::whereIn(ids)->lockForUpdate()->get()`. Dentro valida `activo=true` y `cantidad ≤ stock` (en `unidad_venta` del producto: `M2→cajas`/`Unidad→unidades`); si falla → `DomainException` y `rollback`, carrito permanece intacto.

110. **Cálculos con bcmath (centavos, nunca float)**: `precio_unitario_cents = isM2 ? precioCajaCents() (bcmul precio_cents×m2_por_caja) : precio_cents`; `subtotal_linea = bcmul((string)cantidad, (string)precio_unitario_cents)`; `subtotal = bcadd Σ subtotal_linea`; `ShippingQuote quote = ShippingCalculator->quote(trim(shipping_cp))` (contrato ya existente, Spec 06) → `shipping_cost_cents = quote->disponible ? quote->costoCents : 0` (snapshot, no se recalcula después); `total = bcadd((string)subtotal, (string)shipping_cost_cents)`. `m2_por_caja` `string` nunca `float`.
...
114. **Fuera de alcance Fase 2**: controladores/rutas, `ConfirmPaymentAction`, `Events/Listeners/Jobs`, descuentos, `DTO` si no aporta, UI sobre si checkout debe bloquearse cuando `quote !disponible` (07.2 define `shipping_cost=0`, decisión de rutas/UI no es parte de esta fase).
...
- Concurrencia: dos `PlaceOrder` simultáneos con último stock → `lockForUpdate` serializa, uno `COMMIT`, otro `DomainException` por `cantidad>stock` tras lock.
- `ShippingQuote !disponible` → `shipping_cost=0` snapshot, no excepción; `total = subtotal`.

## Criterios de aceptación (Fase 2)

- [ ] `PlaceOrderAction` existe en `app/Actions/` (una clase = un caso, sin HTTP), inyecta `Cart`, `ShippingCalculator`, `AuditRecorder`.
- [ ] Valida `customer_*`, `shipping_cp`, `payment_method`, `cart` no vacío y `!hasUnpurchasable()`; `DomainException` con mensaje claro y sin side-effects (no `Order`, carrito intacto).
- [ ] Transacción `DB::transaction` + `lockForUpdate` valida `activo` y `cantidad ≤ stock`; si falla → `rollback`.
- [ ] Cálculos `bcmath` centavos (`bcmul`/`bcadd`) para `precio_unitario/subtotal/total`; `m2_por_caja` `string` nunca `float`; `shipping_cost_cents` freeze de `ShippingQuote` (`disponible? costo:0`).
...
- [ ] Tests `tests/Feature/Orders/PlaceOrderTest.php` cubren behaviours: carrito vacío, `activo=false`, `stock insuficiente`, `M2`/`Unidad`, `shipping disponible/no disponible`, snapshot independencia, `audit`, `rollback` carrito intacto vs `commit` carrito limpio, concurrencia/lock (al menos 12–15 tests, la cantidad surge de los behaviours, no fija).
- [ ] Pint, PHPStan nivel 8 (`app/`), Pest verde, CI `lint→stan→test` (una suite `ceramica_test`).

## Decisiones arquitectónicas

- **Una Action = un caso**: `PlaceOrderAction` delgada, sin HTTP, transaccional; reutiliza `Cart`, `M2Calculator` (regla 84) y `ShippingCalculator::quote(): ShippingQuote` (contrato Spec 06 ya existente; 07.2 solo lo consume, no lo rediseña).
...
- **Sin anticipación**: no `ConfirmPaymentAction`, no rutas, no `DTOs/Events/Listeners/Jobs`, no descuentos, no multi-gateway.
...
- [ ] `PlaceOrderAction` + inyección `Cart`/`ShippingCalculator`/`AuditRecorder`.
...
Fase 2 es **solo lógica `PlaceOrderAction`**; no implementar controladores/rutas, pago ni stock. TDD: test red `PlaceOrder` → `lockForUpdate` → `bcmath` → `audit` → `Cart::clear` post-commit. Rama `feat/checkout-07-fase2` desde `main` (post 07.1 merge). Seguir `AGENTS.md`, `.ai/rules`, `PROJECT_PRINCIPLES.md` TDD/ADR, bcmath centavos.
