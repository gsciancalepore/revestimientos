...
├── Contracts/        → puertos / interfaces (PaymentGateway, PaymentStatusQuery)
...
> De las carpetas del árbol, hoy existen: `Actions/`, `Contracts/` (excepción aprobada: puertos externos —`PaymentGateway` y, desde 08.b, `PaymentStatusQuery`—, análogo a `ShippingCalculator` en `Services/`), `Enums/`, `Http/`,
...
`DeleteProductAction` (Spec 03); `PlaceOrderAction` (Spec 07.2, `lockForUpdate` + `bcmath` + snapshot + `Cart::clear` post-commit); `TransitionOrderStatusAction`, `ConfirmPaymentAction` y `CancelOrderAction` (Spec 08.a); `ProcessMercadoPagoNotificationAction` (Spec 08.b). Previsto: `DispatchOrderAction` (Spec 08.c).
`RegisterWhatsAppSaleAction` queda **post-MVP** (Spec 08.2; ver `docs/vision.md` §MVP punto 6,
enmienda 2026-09-12).
...
- **Consulta del estado de un pago** (Spec 08.b): `App\Contracts\PaymentStatusQuery` con `findPayment(string): ?array{status, external_reference, amount_cents}`, implementado por `MercadoPagoGateway`. Puerto **aparte** de `PaymentGateway` porque la transferencia no tiene pago remoto que consultar. Ver §Pedidos — Gestión.
- **Lo que sigue faltando**: la confirmación manual de transferencia desde el panel y las pantallas de pedidos son de la **fase 08.c**; el multi-gateway genérico no está en ninguna spec (YAGNI).

## Pedidos — Gestión (Orders — Spec 08 completa)

Implementado en `docs/specs/08-gestion-pedidos.md:1` (08.a dominio, 08.b webhook, 08.c panel y despacho). Cierra el agujero de que el stock no bajaba nunca y el de que nadie escuchaba a MercadoPago.
...
- **Panel de pedidos** (`OrderController`, reglas 159-162 y 165): `GET /admin/pedidos` con filtro por estado y búsqueda por email o número; `GET /admin/pedidos/{pedido}` con líneas, totales, envío y medio de pago. Los **destacados se derivan del estado, sin columna ni migración** (regla 161): *reposición pendiente* = pedido `paid` con alguna línea cuyo producto quedó en negativo —se apaga sola al reponer—; *incidente de pago* = existe `order.payment_amount_mismatch` o `order.paid_after_cancel` en `audit_logs` —no se apaga, y está bien que no lo haga—. Las auditorías del webhook **no** destacan nada.
- **Autorización por Policy, no por middleware de rol** (`OrderPolicy`): las rutas de pedidos y despacho quedan fuera del grupo `role:admin` porque cada acción tiene su propia fila en la matriz — admin todo; vendedor ve pedidos y no cobra ni cancela; depósito despacha y nunca ve plata. La traza de auditoría es **solo admin** (regla 162: `audit_logs` guarda `ip_address` y `user_agent`). La navegación del panel usa las mismas Policies.
- **Confirmación manual** (regla 159): invoca el mismo `ConfirmPaymentAction` con origen `manual`, y **el guard de "solo transferencia" vive en la Action**, no solo en el panel — la spec lo ubicaba en la pantalla, pero dejarlo ahí repetía el patrón que HIG-06 corrigió en `PlaceOrderAction`.
- **Vista depósito** (`DispatchController`, reglas 163 y 164): `GET /admin/despacho` con dos solapas —*por preparar* (`paid`) y *despachados* (`shipped`)—, ordenadas por antigüedad y **sin importes**. Marca `paid → shipped` y `shipped → delivered` vía `TransitionOrderStatusAction`, que audita. Sin la segunda solapa, la transición que la regla 164 permite al depósito no tendría desde dónde dispararse.
- **Stock negativo** (regla 146): público (catálogo, ficha, carrito) trata `stock <= 0` como **sin stock** y nunca muestra un número negativo; el panel muestra el valor real, porque es cuánto hay que reponer. `UpdateProductRequest` deja de validar `min:0` —con −3 el admin no podía guardar ningún cambio del producto—; `StoreProductRequest` lo conserva. El modelo anota `@property int $stock` **con signo**: la columna es `unsignedInteger` y el análisis estático infería `int<0, max>`, dando `stock < 0` por imposible.
- **Sin anticipación**: sin ventas por WhatsApp (Spec 08.2, **post-MVP** desde 2026-09-12), sin `Events`/`Listeners`/`Jobs`, sin movimientos de stock como entidad (ADR-005 → Fase 3), sin marca de "incidente atendido" (haría falta columna y migración para un flujo que no existe).
