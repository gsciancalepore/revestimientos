...
| 1e | Observabilidad: ADR-013 y spec `observabilidad-01-contrato-logs` (contrato de logs v1 para el investigador de incidentes externo) | 🔄 **implementada** (2026-09-24), rama `feat/observabilidad-01-contrato-logs`: JSON por línea validado contra `docs/observabilidad/log-schema.v1.json`, `request_id`, catálogo de eventos, redacción de datos personales en todos los canales, excepciones sin texto libre. Enmienda las reglas 124, 150 y 151. 554 tests. `revisor-entrega` bloqueó la primera versión por un test del criterio 1 que miraba una sola línea y por 8 mutaciones que sobrevivían; corregido, todas en rojo. Falta el PR y, en local, `LOG_CHANNEL=app` y borrar `laravel.log` |
