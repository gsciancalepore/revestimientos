...
### Punto de retome — 2026-09-24: contrato de logs v1 implementado

**Leer esto primero.** Hay tres ramas en vuelo, en este orden de merge:

1. `feat/catalogo-04-regla-167-home`: regla 167, lista para PR.
2. `docs/observabilidad-borrador`: ADR-013 y la spec `observabilidad-01-contrato-logs`, aprobadas
   por el dueño, listas para PR.
3. `feat/observabilidad-01-contrato-logs`: la implementación, creada encima de la 2. Falta
   `revisor-entrega` y su PR.

Al mergear la 3:
- copiar `docs/observabilidad/log-schema.v1.json` en
  `~/incident-investigator/contracts/revestimientos/`, anotando el commit de origen;
- en local, poner `LOG_CHANNEL=app` en `.env` y borrar `storage/logs/laravel.log`. La imagen `app`
  ya está reconstruida con `zend.exception_ignore_args`.

Siguen pendientes la identidad visual (Fases B y C) y la verificación del webhook contra MercadoPago
real.
