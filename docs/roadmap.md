...
| H03 | Higiene 03 — oferta, calculadora y cobertura | La oferta se cobra, calculadora única, roturas 500 y cobertura | Products + Orders + Payments | 🟡 **03.a mergeada** (PR #30, 462 tests); **03.b implementada y auditada** (rama `fix/higiene-03b`, **499 tests**, `revisor-entrega` apto): HIG-23–HIG-32, solo tests + índices únicos + guard ADMIN_EMAIL. Falta push, PR y merge |
...
### Punto de retome — 2026-09-17: fase 03.b implementada, falta revisión y PR

**Leer esto primero.** La fase **03.b está implementada** en la rama `fix/higiene-03b` (cortada desde
`main` con 03.a mergeada): HIG-23 a HIG-32 —índices únicos en `categories` con fallo legible (la
única que toca esquema), tests de `UserPolicy` sin middleware, throttle, reset, `DatabaseSeeder`
con `ADMIN_*` en `phpunit.xml` y guard que nombra la variable, sidebar por rol, tarjetas y orden
del catálogo, los dos tests de la Spec 02, mínimo de 8 en los tres caminos y los 5 items de HIG-32—.
Verificación local completa (Pint, PHPStan 8, **499 tests**), mutación regla por regla y
`revisor-entrega` (**apto**, con 2 observaciones menores ya aplicadas: warning de import y commit de
regla fuera de alcance movido a su rama). Falta: push y PR a `main` (a pedido del dueño).
