...
| 1e | Observabilidad: ADR-013 y spec `observabilidad-01-contrato-logs` (contrato de logs v1 para el investigador de incidentes externo) | ✅ **cerrada** (2026-09-24, PR #39, `b8c2ba0`): contrato de logs v1 (JSON por línea validado contra `docs/observabilidad/log-schema.v1.json`, `request_id`, catálogo de eventos, redacción de datos personales en todos los canales, excepciones sin texto libre). Enmienda las reglas 124, 150 y 151. 559 tests. `revisor-entrega` bloqueó la primera versión por cobertura y dio apto en la segunda. Las etapas 2 a 5 de ADR-013 no están comprometidas |
...
### Punto de retome — 2026-09-24 (noche): observabilidad 01 cerrada

**Leer esto primero.** No hay ramas en vuelo en este repo: #37 (regla 167), #38 (ADR-013 y spec) y
#39 (contrato de logs v1) están mergeados.

**Qué quedó**:
- Revestimientos escribe `storage/logs/app-AAAA-MM-DD.jsonl` según el contrato de
  `docs/observabilidad/`. Leer su README antes de tocar cualquier `Log::`, y
  `.ai/rules/observabilidad.md`.
- El investigador de incidentes es **otro repositorio**, `~/incident-investigator`, con docs en
  inglés, Python y LangGraph, y el plan gratuito de Gemini como modelo por defecto. Tiene la
  arquitectura v1 definida en `docs/architecture.md` y 4 ADRs, y ya tiene el contrato copiado. **No
  tiene código ni remoto en GitHub todavía.** Nunca se implementa como agente dentro de este repo
  (ADR-013).

**Lo próximo, a elección del dueño**:

1. **`incident-investigator` v1**, en su repo. Antes de escribir código:
   - crear el repo en GitHub y la `GOOGLE_API_KEY`;
   - reproducir en Revestimientos los escenarios de evaluación (`docs/architecture.md` §Evaluation) y
     commitear sus logs como fixtures del investigador.
2. Identidad visual Fase B (home + catálogo, incluida la sección "Productos" de la regla 167, que
   todavía usa la paleta `stone`/`orange`) y Fase C (ficha + carrito + checkout).
3. **Verificación del webhook contra MercadoPago real** (único pendiente del MVP).
4. Etapa 2 de ADR-013 (logs de staging): necesita spec propia.

**Pendientes menores anotados**:
- `composer audit`: 4 advisories de `league/commonmark`, previos a esta etapa.
- El contenedor local no tiene `ext-sockets`: los `composer require` necesitan
  `--ignore-platform-req=ext-sockets`.

### Punto de retome — 2026-09-24 (tarde): contrato de logs v1 en PR #39

**Leer esto primero.** Ya se mergearon #37 (regla 167) y #38 (ADR-013 y la spec
`observabilidad-01-contrato-logs`). Falta **#39**, la implementación
(`feat/observabilidad-01-contrato-logs`), que integró `main` y tiene `revisor-entrega` apto.

Al mergear #39, copiar `docs/observabilidad/log-schema.v1.json` en
`~/incident-investigator/contracts/revestimientos/`, anotando el commit de origen. En la máquina del
dueño ya están hechos `LOG_CHANNEL=app` en `.env`, el borrado de `storage/logs/laravel.log` y la
imagen `app` reconstruida.

**Lo próximo**:

1. Arrancar la implementación de `incident-investigator` v1, que es otro repositorio.
2. Identidad visual Fase B (home + catálogo, incluida la sección "Productos" de la regla 167, que
   todavía usa la paleta `stone`/`orange`) y Fase C (ficha + carrito + checkout).
3. **Verificación del webhook contra MercadoPago real** (único pendiente del MVP).
